		...
		if (isReturn == true) {
			if (dwTransferred != 0) {
				if (lpCompletionKey != 0 && overlapped != 0) {
					...
					case ASYNCFLAG_RECV:
						DispatchRecv(dwTransferred);
						break;
					case ASYNCFLAG_SEND:
						DispatchSend(dwTransferred);
						break;
					}
				}
			}
...
int PP::PPIOCP::DispatchRecv(DWORD dwTransferred) {
	int iReturn = 0;
	LARGE_INTEGER lr;
	...
	//PPReceivePacketPool::GetInstance().push_back(packetRecv);
...
int PP::PPIOCP::DispatchSend(DWORD dwTransferred) {
	LARGE_INTEGER lr;
	lr.QuadPart = dwTransferred;
